...
if (modal_ok)
	modal_ok.addEventListener('click', () => modal.classList.remove('show'))
...
function format_uptime(uptime_secs) {
	...
	if (months > 0)
		formatted_uptime += `${months} month${months > 1 ? 's' : ''}, `
	...
	const formatted_time = new Date(new Date().getTime() - uptime_secs * 1000)
		...
		.reverse()
		.join('-')
	formatted_uptime += ` (${formatted_time.substring(
		formatted_time.indexOf(' ') + 1
	)})`
	return `Since ${formatted_uptime}`
}

let previous_href = window.location.href,
	uptime_secs = parseInt(sessionStorage.getItem('srv_uptime')),
	refresh_ticker = 0
setInterval(() => {
	const current_href =
			...
			case 'Setting.html':
				{
					...
					if (licence_key && licence_day) {
						licence_key.innerHTML = sessionStorage
							.getItem('licence_key')
							.replace(/(\d{4})(?=\d)/g, '$1 ')
			...
			case 'Home.html':
				{
					if (sessionStorage.getItem('licence_days') <= 0) {
						setTimeout(
							() =>
								show_modal(
									'Please note that there are no days remaining on your licence. If you wish to continue using BetterTelegram, then head over to the settings page & add more days to your licence. </br> Thank you!'
								),
							3333
						)
					}
